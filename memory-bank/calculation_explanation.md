...
*   **Прямые расчеты/визуализации на странице:**
    *   **Расчеты:** Выполняет основные расчеты (`generateCashFlow`, `aggregateReportData`, `generatePnL`) и передает результаты дочерним компонентам. // Централизованные расчеты.
    *   `FinancialReport`: **Принимает props** (`aggregatedReportData`, `totalPnlData`, `currentParams`). Отображает **сводную** таблицу месячного Cash Flow (Доходы, Расходы, Чистый денежный поток, Накопленный поток), извлекая/рассчитывая данные из `aggregatedReportData`. Отображает **итоговые** P&L данные из `totalPnlData`. // Отображение сводных CF и итоговых P&L из props.
    *   `HorizontalCashflowReport`: **Принимает props** (`aggregatedReportData`, `modelingYear`). Отображает **детализированную** иерархическую таблицу месячного/квартального Cash Flow на основе `aggregatedReportData`. // Отображение детального CF из props.
...
*   **Логика:**
    *   **Получение данных:** Компонент **принимает props** `aggregatedReportData` (рассчитанные в `FinancialModelingPage`) и `modelingYear`. // Получение готовых данных через props.
    *   **Агрегация (`aggregateReportData`):** Логика агрегации находится в `reportUtils.ts` и **вызывается в родительском компоненте** (`FinancialModelingPage`). Эта функция преобразует сырые данные CF (`baseCashFlowData`) и списки затрат/персонала в структурированный формат `ReportRow[]`. // Логика агрегации вынесена.
    *   **Переключение периода:** Позволяет пользователю выбрать месячный или квартальный вид. // Переключатель Месяц/Квартал.
    *   **Отображение:** Рендерит таблицу MUI, используя **проп `aggregatedReportData`** и `getPeriodValue` для отображения сумм за месяц или квартал. Применяет стили для иерархии и выделения строк. // Вывод таблицы на основе props.
...
*   **Влияющие параметры:** Косвенно - все параметры, влияющие на расчеты в `FinancialModelingPage`, т.к. компонент отображает результаты этих расчетов, переданные через props.
