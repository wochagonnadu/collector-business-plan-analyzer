...
const StageForm: React.FC<StageFormProps> = ({ initialValues, onClose }) => {
  ...
  const formInitialValues: Omit<Stage, 'id' | 'subStages'> = initialValues
    ? {
  ...
  const handleSubmit = (values: Omit<Stage, 'id' | 'subStages'>) => {
    const payload = { ...values, dependsOn: values.dependsOn || [] };

    if (isEditing && initialValues) {
      const updatedStage: Stage = { ...initialValues, ...payload };
      dispatch(updateStage(updatedStage));
      console.log('Обновление этапа:', updatedStage);
    } else {
      dispatch(addStage(payload));
      console.log('Добавление этапа:', payload);
    }
    onClose();
  };
