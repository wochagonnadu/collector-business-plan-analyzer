...
export interface DebtPortfolio {
  ...
  // // Добавляем стандартное отклонение для суммы долга (для симуляции)
  averageDebtSigma?: number; // // Стандартное отклонение от средней суммы долга (например, 20)
  // // Добавляем дату начала проекта/портфеля
  startDate?: string; // // Дата в формате YYYY-MM-DD
  // // Флаг, указывающий, является ли это первоначальной покупкой портфеля
  isInitialPurchase?: boolean; // // true, если это первая покупка при старте
}
...
export interface FinancialParams {
  ...
  // // Добавляем флаг для выбора режима уплаты налога на прибыль
  payTaxesMonthly?: boolean; // // true = ежемесячно, false/undefined = ежеквартально (по умолчанию)
  // // Добавляем ставку комиссии для переменных затрат на персонал
  variableCommissionRate: number; // // Ставка комиссии (например, 0.05 для 5%)
